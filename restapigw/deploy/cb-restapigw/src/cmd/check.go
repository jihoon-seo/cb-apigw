...
func printConfigurations(cmd *cobra.Command, sConf config.ServiceConfig) {
	...
	cmd.Printf("Endpoints (%d):\n", len(sConf.Endpoints))
	for _, endpoint := range sConf.Endpoints {
		cmd.Printf("\tEndpoint: %s, Method: %s, CacheTTL: %s, Except QueryString: %v, Except Headers: %v\n",
			endpoint.Endpoint, endpoint.Method, endpoint.CacheTTL.String(), endpoint.ExceptQueryStrings, endpoint.ExceptHeaders)
