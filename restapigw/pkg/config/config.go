...
const (
	...
	// DefaultCacheTTL - 기본 값으로 사용할 Cache TTL 시간
...
func InitDefinitions(sConf *ServiceConfig, defs []*EndpointConfig) error {
	...
	for _, def := range defs {
		...
		inputParams := core.ExtractPlaceHoldersFromURLTemplate(def.Endpoint, core.ParameterExtractPattern(sConf.DisableStrictREST))
		inputSet := map[string]interface{}{}
		for ip := range inputParams {
			inputSet[inputParams[ip]] = nil
		}

		def.Endpoint = core.GetParameteredPath(def.Endpoint, inputParams)
